...
(
  ...
  // ── RVV Extension Interface ───────────────────────────────────────────────
  input  logic        rvv_stall_i,          // from decoder: vector instruction detected
  input  logic        rvv_done_i,           // from RVV unit: operation complete
  output logic        rvv_valid_o,          // to RVV unit: operands valid, begin execution
  output logic        rvv_wb_en_o,          // to RVV unit: this instruction writes back to rd
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // RVV handshake registers
  logic rvv_wb_en_q;   // latched: does this vector instruction need rd writeback?
  ...
  begin
    ...
    rvv_valid_o  = 1'b0;
    rvv_wb_en_o  = rvv_wb_en_q;   // hold the captured value throughout RVV_WAIT
    ...
    unique case (ctrl_fsm_cs)
              ...
              begin
                ...
                if (illegal_insn_i) begin
                  ...
                  unique case (1'b1)
                    ...
                    rvv_stall_i: begin
                      // Freeze IF so no new instruction enters.
                      // Do NOT freeze ID yet — we need the instruction to
                      // move to EX so RS1/RS2 land on the EX-stage operand
                      // wires where the RVV unit will sample them.
                      halt_if_o    = 1'b1;

                      // Tell the RVV unit that operands are on their way
                      // (they will be valid in EX on the very next cycle).
                      rvv_valid_o  = 1'b1;

                      // Forward the writeback intent to the RVV unit.
                      // regfile_we_id_i is the decoded WE for rd; we must
                      // capture it here before deassert_we_o kills it.
                      rvv_wb_en_o  = regfile_we_id_i;

                      // Wait for the instruction to finish moving from
                      // ID → EX before entering the wait state.
                      ctrl_fsm_ns  = id_ready_i ? RVV_WAIT : DECODE;
                    end
                    // ────────────────────────────────────────────────────────
      ...
      //──────────────────────────────────────────────────────────────────────
      // Core is frozen, waiting for the RVV unit to finish.
      // The vector instruction's operands (RS1/RS2) are sitting in the EX
      // stage pipeline register; the RVV unit reads them through its own
      // dedicated port.  If it needs to write back into rd, rvv_wb_en_o
      // tells it so, and the RVV unit drives the regfile write port directly
      // when it asserts rvv_done_i.
      //──────────────────────────────────────────────────────────────────────
      RVV_WAIT:
      begin
        is_decoding_o = 1'b0;

        // Both stages are frozen: IF cannot fetch and ID cannot accept a new
        // instruction until the vector operation finishes.
        halt_if_o     = 1'b1;
        halt_id_o     = 1'b1;

        // rvv_valid_o is already 0 (set in defaults); the RVV unit latched
        // it on the first cycle.  rvv_wb_en_o keeps its registered value.

        if (rvv_done_i) begin
          // Vector op finished.  If writeback is needed, the RVV unit is
          // driving the regfile write port externally right now.
          // Release the pipeline and go back to decode.
          ctrl_fsm_ns = DECODE;
        end else begin
          ctrl_fsm_ns = RVV_WAIT;

          // Allow IRQ or debug to abort the wait if needed.
          // (Optional — add irq/debug preemption here if your design requires it.)
        end
      end
      //──────────────────────────────────────────────────────────────────────
...
endgenerate
  ...
  begin
    load_stall_o   = 1'b0;
    // ── Vector instruction stall ─────────────────────────────────────────
    // While a vector instruction is in ID or the core is in RVV_WAIT,
    // suppress the scalar register-file write-enable completely.
    // The RVV unit owns any writeback to rd during this window.
    if (rvv_stall_i || (ctrl_fsm_cs == RVV_WAIT)) begin
      deassert_we_o = 1'b1;
    end
    // ─────────────────────────────────────────────────────────────────────
    deassert_we_o  = 1'b0;
  ...
  begin : UPDATE_REGS
    ...
    begin
      ...
      // Capture the writeback intent exactly when the vector instruction
      // is accepted into EX (the cycle we leave DECODE for RVV_WAIT).
      if (rvv_stall_i && instr_valid_i &&
          (ctrl_fsm_cs == DECODE) && id_ready_i) begin
        rvv_wb_en_q <= regfile_we_id_i;
      end else if (ctrl_fsm_cs == RVV_WAIT && rvv_done_i) begin
        rvv_wb_en_q <= 1'b0;   // clear when done
      end
